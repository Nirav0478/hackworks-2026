...
check("shower_gallons = 10 * 7 * 2.1 = 147 gal", abs(w["shower_gallons"] - 147.0) < 0.01)
check("bathtubs > 0",                             w["bathtubs"] > 0)
check("1 min shower, 1 shower → 2.1 gal",         abs(calculate_water(1, 1)["shower_gallons"] - 2.1) < 0.01)
...
check("saved_drive_yearly = saved_drive * 52",    abs(s["saved_drive_yearly"] - s["saved_drive"] * 52) < 0.01)
check("saved_water_food > 0",                     s["saved_water_food"] > 0)
check("saved_shower > 0 (10 min > 7 min target)", s["saved_shower"] > 0)
check("shower at 5 min → saved_shower = 0",       calculate_savings(0, 0, 5, 7, 0)["saved_shower"] == 0)
check("saved_ac_yearly = saved_ac * 52",          abs(s["saved_ac_yearly"] - s["saved_ac"] * 52) < 0.01)

print("\n── Edge Cases ───────────────────────────")
check("all zeros → no crashes", (
    calculate_driving(0, 0) is not None and
    calculate_food(0, 0) is not None and
    calculate_water(0, 0) is not None and
    calculate_energy(0, 0) is not None
))
